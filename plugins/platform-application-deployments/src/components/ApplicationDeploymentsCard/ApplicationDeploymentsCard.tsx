...
export const DenseTable = ({apps}: DenseTableProps) => {
    const columns: TableColumn<App>[] = [
        {
            title: 'Applications', field: 'appName', render: (data) => (
                <IconLink href={data.appUrl} text={data.appName} Icon={data.appType=== AppType.BWCE ? BwceSvg: FlogoSvg}/>
            )
        },
        {
            title: 'Data planes', field: 'dataPlaneName', render: (data) => (

                <IconLink href={data.dataPlaneUrl} text={data.dataPlaneName} Icon={DpSvg}/>
            )
        }
    ];
...
export const ApplicationDeploymentsCard = () => {
    try {
        ...
        if (!cpLink) {
            return <ResponseErrorPanel error={new Error('CP link not found')}></ResponseErrorPanel>
        }
        ...
        const inputApps: InputApp[] = entity?.metadata?.tibcoPlatformApps as unknown as InputApp[] || [];
        const apps: App[] = inputApps.map((app) => {
            const dataPlaneUrl = `${cpLink}cp/app/data-plane?dp_id=${app.dpId}`;
            const appUrl = `${cpLink}cp/${app.appType.toLowerCase()}/appdetails?dp_id=${app.dpId}&capability_instance_id=${app.capabilityInstanceId}&app_id=${app.appId}`;
            return {...app, dataPlaneUrl, appUrl}
        })

        return <DenseTable apps={apps || []}/>;
    } catch (err) {
        return <ResponseErrorPanel error={err as Error}></ResponseErrorPanel>
    }
